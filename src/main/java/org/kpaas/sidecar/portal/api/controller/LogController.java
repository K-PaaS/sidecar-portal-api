...
public class LogController {
    ...
    public Map getRecentLog(@PathVariable String appGuid) throws Exception {
        ...
        try {
            Batch respAppLogs = logService.getLog(appGuid, "-6795364578871345152", 100, true, "LOG");
    ...
    public Map getTailLog(@PathVariable String appGuid, String token) throws Exception {
        Map mapLog = new HashMap();
        try {
            Batch respAppLogs = logService.getLog(appGuid, "-6795364578871345152", 1, true, "LOG");
            mapLog.put("log", respAppLogs);
        } catch (Exception e) {
            mapLog.put("log", "");
        }
    ...
    public Map getTailLog(@PathVariable String appGuid, @PathVariable String time, String token) throws Exception {
        Map mapLog = new HashMap();
        try {
            Batch respAppLogs = logService.getLog(appGuid, time,0, false, "LOG");
            mapLog.put("log", respAppLogs);
        } catch (Exception e) {
            mapLog.put("log", "");
        }

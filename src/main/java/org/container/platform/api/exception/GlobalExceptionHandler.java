...
public class GlobalExceptionHandler extends RuntimeException {
    ...
    @ExceptionHandler({ClientV3Exception.class})
    public ErrorMessage cfException(final ClientV3Exception ex) {
        LOGGER.info( CommonUtils.loggerReplace(ex.getClass()) + "  Exception >>> {}",   CommonUtils.loggerReplace(ex.getLocalizedMessage()));
        return new ErrorMessage(Constants.RESULT_STATUS_FAIL, ex.getLocalizedMessage(), ex.getStatusCode(), ex.getLocalizedMessage());
    }
